		...
		<div class="card-body">
			<table class="table tabe-hover table-bordered" id="list">
				...
				<tbody>
					<?php
					$i = 1;
					$qry = $conn->query("SELECT * FROM domain order by id asc");
					while($row= $qry->fetch_assoc()):
					?>
					<tr>
						<th class="text-center"><?php echo $i++ ?></th>
						<td><b><?php echo $row['dep_name'] ?></b></td>
...
<script>
	$(document).ready(function(){
		...
		$('.new_class').click(function(){
			uni_modal("New class","manage_department.php")
		})
		$('.manage_class').click(function(){
			uni_modal("Manage class","manage_department.php?id="+$(this).attr('data-id'))
		})
	$('.delete_class').click(function(){
	_conf("Are you sure to delete this department?","delete_class",[$(this).attr('data-id')])
	})
	})
